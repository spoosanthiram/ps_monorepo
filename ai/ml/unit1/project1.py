...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    Returns:
        ...
        parameters.
    """
    return max(0.0, 1.0 - label * (feature_vector.dot(theta) + theta_0))
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    y = np.maximum(0.0, 1.0 - (np.matmul(feature_matrix, theta) + theta_0) * labels)
...
def perceptron_single_step_update(
    ...
    """
    if (current_theta.dot(feature_vector) + current_theta_0) * label <= 0.0:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            theta, theta_0 = perceptron_single_step_update(feature_matrix[i], labels[i], theta, theta_0)
            summed_theta += theta
            summed_thea_0 += theta_0
    n = T * feature_matrix.shape[0]
    return (summed_theta / n, summed_thea_0 / n)
...
def pegasos_single_step_update(
    ...
    """
    multiplier = (1 - eta * L)
    if (theta.dot(feature_vector) + theta_0) * label <= 1:
        theta = multiplier * theta + (eta * label) * feature_vector
        theta_0 = theta_0 + eta * label
        return (theta, theta_0)
    else:
        return (multiplier * theta, theta_0)
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    return 2 * np.heaviside((feature_matrix @ theta) + theta_0, 0) - 1
...
def extract_bow_feature_vectors(reviews, indices_by_word, binarize=True):
    ...
    if binarize:
        feature_matrix = np.heaviside(feature_matrix, 0)
